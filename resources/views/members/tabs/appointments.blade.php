...
<div class="mg-tab-block">
    <div class="mg-tab-block__head">
        <div>
            <p class="mg-section-label mb-1">Agenda</p>
            <h2 class="mg-tab-block__title">Agendamentos</h2>
        </div>
        <div class="mg-tab-actions">
            <a href="{{ route('events.create', ['member' => $member->id]) }}" class="mg-btn-primary">
                <i class="ri-add-line"></i> Agendamento único
            </a>
            <a href="{{ route('events.create', ['member' => $member->id]) }}" class="mg-btn-ghost">
                <i class="ri-calendar-event-line"></i> Agendar para o período
            </a>
            <a href="{{ route('members.show', [$member, 'tab' => 'appointments', 'period' => 'future']) }}" class="mg-btn-ghost">
                <i class="ri-calendar-check-line"></i> Ver futuros
            </a>
        </div>
    </div>

    <form method="GET" action="{{ route('members.show', [$member, 'tab' => 'appointments']) }}" class="mg-tab-filters mb-3">
        <input type="hidden" name="tab" value="appointments">
        <div>
            <label class="mg-field-label">Período</label>
            <select name="period" class="mg-field" onchange="this.form.submit()">
        ...
        <div>
            ...
            <select name="appointment_status" class="mg-field" onchange="this.form.submit()">
        ...
        <div>
            ...
            <select name="appointment_type" class="mg-field" onchange="this.form.submit()">
        ...
        <div class="mg-tab-filters__actions">
            <a href="{{ route('members.show', [$member, 'tab' => 'appointments']) }}" class="mg-btn-ghost">Limpar</a>
        </div>
    </form>
    ...
    @forelse($filteredAppointments as $event)
        ...
        <article class="mg-prescription-card">
            <div class="mg-prescription-card__main">
                <div class="mg-prescription-card__eyebrow">{{ $event->start_time?->format('d/m/Y H:i') }}</div>
                <h3 class="mg-prescription-card__title">{{ $event->title }}</h3>
            ...
            <div class="mg-card-actions">
                <span class="mg-chip {{ $statusClass }}">{{ $event->status === 'scheduled' ? 'Pendente' : $statusLabel }}</span>
                <a href="{{ route('events.edit', $event) }}" class="mg-icon-btn" title="Editar"><i class="ri-pencil-line"></i></a>
                <form method="POST" action="{{ route('events.destroy', $event) }}" onsubmit="return confirm('Excluir este agendamento?')">
                    @csrf
                    @method('DELETE')
                    <button class="mg-icon-btn mg-btn-danger-ghost" title="Excluir"><i class="ri-delete-bin-line"></i></button>
                </form>
            </div>
        </article>
    @empty
        <div class="mg-empty-state mg-empty-state--compact">
            ...
            <a href="{{ route('events.create', ['member' => $member->id]) }}" class="mg-btn-primary">Agendamento único</a>
        </div>
    @endforelse
    </div>
</div>
