...
@push('styles')
    <style>
        ...
        .prime-menu-card__meals {
            display: grid;
            gap: 0.36rem;
            margin-top: 0.66rem;
        }

        .prime-menu-card__meal {
            display: grid;
        ...
        .prime-menu-card__meal-title {
            color: #1A2840;
            font-size: 0.78rem;
            font-weight: 880;
        }

        .prime-menu-card__meal-foods {
            overflow: hidden;
            color: #7C8BA0;
            font-size: 0.7rem;
            font-weight: 650;
        ...
        @media (max-width: 1199.98px) {
            ...
            .prime-menu-builder__top > :first-child,
            .prime-menu-builder__top > :nth-child(2),
            .prime-menu-food-row > :first-child,
            .prime-menu-food-row > :nth-child(4) {
                grid-column: 1 / -1;
            }
        }
...
@section('content')
<div class="prime-clients-page prime-menu-page">
    ...
    <form method="POST" action="{{ route('library.diet.menus.store') }}" class="prime-menu-builder" data-menu-builder-form>
        @csrf
        ...
        <div class="prime-menu-builder__top">
            <div>
                <label class="prime-field-label">Nome do cardápio</label>
                <input name="name" class="prime-field" value="{{ old('name') }}" placeholder="Ex: Hipertrofia 2.800 kcal" required>
            </div>
            <div>
                <label class="prime-field-label">Descrição</label>
                <input name="description" class="prime-field" value="{{ old('description') }}" placeholder="Objetivo, restrições, observações">
            </div>
            <div>
            ...
            <div>
                ...
                <div class="prime-menu-total-chip"><span data-menu-meals-count>0</span> itens</div>
            </div>
            <div>
                <button class="prime-btn-primary w-100" type="submit">
                    <i class="ri-save-line"></i> Salvar
                </button>
            </div>
        </div>

        <div class="prime-menu-metrics">
            <div class="prime-menu-metric">
                <div class="prime-menu-metric__label">Calorias</div>
                <div class="prime-menu-metric__value"><span data-menu-total-kcal>0</span> kcal</div>
            </div>
            <div class="prime-menu-metric prime-menu-metric--protein">
                <div class="prime-menu-metric__label">Proteína</div>
                <div class="prime-menu-metric__value"><span data-menu-total-protein>0,0</span>g</div>
            </div>
            <div class="prime-menu-metric prime-menu-metric--carbs">
                <div class="prime-menu-metric__label">Carbo</div>
                <div class="prime-menu-metric__value"><span data-menu-total-carbs>0,0</span>g</div>
            </div>
            <div class="prime-menu-metric prime-menu-metric--fat">
                <div class="prime-menu-metric__label">Gordura</div>
                <div class="prime-menu-metric__value"><span data-menu-total-fat>0,0</span>g</div>
            </div>
        </div>

        <div class="d-flex flex-wrap align-items-center justify-content-between gap-2 mb-2">
            <div>
                <div class="prime-panel-label mb-1">Refeições reais</div>
                <p class="prime-panel-hint mb-0">O total usa a fórmula (macro/100) × gramas, sem refresh.</p>
            </div>
            <button type="button" class="prime-btn-ghost" data-add-menu-meal @disabled($foods->isEmpty())>
                <i class="ri-add-line"></i> Adicionar refeição
            </button>
        </div>

        @if($foods->isEmpty())
            <div class="prime-empty-state py-3">
                <i class="ri-apple-line"></i>
                <p>Cadastre alimentos antes de montar cardápios reais.</p>
                <a href="{{ route('library.diet.foods') }}" class="prime-btn-primary">Cadastrar alimento</a>
            </div>
        @endif

        <div class="prime-menu-meals" data-menu-meals></div>
    </form>

    <div class="prime-menu-list">
        @forelse($menus as $menu)
            <div class="prime-menu-card">
                <div class="prime-menu-card__head">
                    <div class="min-w-0">
                        <div class="prime-menu-card__title">{{ $menu->name }}</div>
                        <div class="prime-menu-card__meta">
    ...
    <script>
        (function () {
            ...
            function foodOptions(selectedId = '') {
                if (!dietFoods.length) {
                    return '<option value="">Cadastre alimentos primeiro</option>';
                }
            ...
            function foodTemplate(mealIndex, foodIndex, foodItem = {}) {
                return `
                    <div class="prime-menu-food-row" data-menu-food-row>
                        <select name="meals[${mealIndex}][foods][${foodIndex}][diet_food_id]" class="prime-field prime-field--sm" data-menu-food-select required>
                            ${foodOptions(foodItem.diet_food_id)}
                        </select>
                        <input name="meals[${mealIndex}][foods][${foodIndex}][quantity_in_grams]" type="number" min="1" step="1" class="prime-field prime-field--sm" value="${escapeAttribute(foodItem.quantity_in_grams ?? 100)}" data-menu-food-grams required>
                        <div class="prime-menu-row-macros" data-menu-row-macros>0 kcal · P 0,0g · C 0,0g · G 0,0g</div>
                        <input name="meals[${mealIndex}][foods][${foodIndex}][notes]" class="prime-field prime-field--sm" value="${escapeAttribute(foodItem.notes)}" placeholder="Notas">
                        <button type="button" class="prime-icon-btn" data-remove-menu-food title="Remover alimento">
            ...
            function mealTemplate(mealIndex, meal = {}) {
                return `
                    <div class="prime-menu-meal" data-menu-meal data-meal-index="${mealIndex}">
                        <div class="prime-menu-meal__head">
                            <div>
                                <label class="prime-field-label">Refeição</label>
                                <input name="meals[${mealIndex}][name]" class="prime-field prime-field--sm" value="${escapeAttribute(meal.name)}" placeholder="Ex: Café da manhã" required>
                            </div>
                            <div>
                                <label class="prime-field-label">Horário</label>
                                <input name="meals[${mealIndex}][time_label]" class="prime-field prime-field--sm" value="${escapeAttribute(meal.time_label)}" placeholder="07:00">
                            </div>
                            <div>
                                <label class="prime-field-label">Notas</label>
                                <input name="meals[${mealIndex}][notes]" class="prime-field prime-field--sm" value="${escapeAttribute(meal.notes)}" placeholder="Pré/pós, substituições...">
                            </div>
                            <span class="prime-menu-total-chip" data-menu-meal-total>0 kcal · P 0,0g · C 0,0g · G 0,0g</span>
                            <button type="button" class="prime-icon-btn" data-remove-menu-meal title="Remover refeição">
                                <i class="ri-delete-bin-line"></i>
                            </button>
                        </div>
                        <div class="prime-menu-foods" data-menu-foods data-next-food-index="0"></div>
                        <button type="button" class="prime-btn-ghost mt-2" data-add-menu-food>
