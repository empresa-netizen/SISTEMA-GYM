...
@php
    ...
    $statusChip = [
        'draft' => '',
        'published' => 'prime-chip--success',
        'archived' => 'prime-chip--warn',
    ];
@endphp
...
<div class="prime-clients-page">
    <div class="prime-clients-toolbar">
        <div class="prime-clients-toolbar__left">
            <h1 class="prime-page-title mb-0">Meus Cursos</h1>
            <p class="prime-page-sub mb-0">Gerencie seus cursos, módulos e aulas</p>
        </div>
        <div class="prime-clients-toolbar__right">
            <button type="button" class="prime-btn-primary" data-bs-toggle="modal" data-bs-target="#courseModal">
    ...
    <form method="GET" action="{{ route('library.courses') }}" class="prime-panel prime-panel--compact">
        <div class="row g-2 align-items-end">
            <div class="col-xl-4 col-md-6">
                <label class="prime-field-label">Buscar</label>
                <input type="search" name="q" value="{{ $filters['q'] }}" class="prime-field" placeholder="Buscar curso, módulo ou aula">
            </div>
            ...
            <div class="col-xl-2 col-md-6">
                <label class="prime-field-label">Ordenar</label>
                <select name="sort" class="prime-field">
            ...
            <div class="col-xl-2 col-md-12 d-flex gap-2">
                ...
                @if($hasFilters)
                    <a href="{{ route('library.courses') }}" class="prime-btn-ghost" title="Limpar filtros">
        ...
        @forelse($courses as $course)
            <div class="prime-client-card">
                <div class="prime-client-card__main">
                    <div class="prime-client-card__avatar" style="background:linear-gradient(135deg,#4f46e5,#8b5cf6)">
                        <i class="ri-graduation-cap-line"></i>
                    </div>
                    <div class="prime-client-card__identity">
                        <div class="prime-client-card__name">{{ $course->title }}</div>
                        ...
                        <div class="prime-client-chips">
                            <span class="prime-chip {{ $statusChip[$course->status] ?? '' }}">{{ $statusOptions[$course->status] ?? $course->status }}</span>
                        </div>
                    </div>
                </div>
                <div class="prime-client-card__actions">
                    <span class="prime-chip" title="Pronto para vincular a um produto/cliente">Biblioteca</span>
                </div>
            </div>
        @empty
            <div class="prime-empty-state">
                ...
                <button type="button" class="prime-btn-primary" data-bs-toggle="modal" data-bs-target="#courseModal">
                    <span aria-hidden="true">+</span> Criar Primeiro Curso
                </button>
            </div>
        @endforelse
    </div>
    ...
    @if(method_exists($courses, 'hasPages') && $courses->hasPages())
        <div class="prime-pagination">{{ $courses->links() }}</div>
    @endif
</div>
