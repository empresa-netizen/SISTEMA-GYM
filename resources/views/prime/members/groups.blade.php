...
<div class="prime-clients-page">
    <div class="prime-clients-toolbar">
        <div class="prime-clients-toolbar__left">
            <h1 class="prime-page-title mb-0">Meus grupos</h1>
    ...
    <p class="prime-page-sub mb-0">Organize comunidades e acompanhe os grupos cadastrados com dados locais.</p>

    <form method="GET" class="prime-panel prime-panel--compact">
        <div class="row g-2 align-items-end">
            <div class="col-xl-3 col-md-6">
                <label class="prime-field-label">Buscar grupo</label>
                <input type="search" name="q" value="{{ $filters['q'] }}" class="prime-field" placeholder="Nome ou descrição">
            </div>
            <div class="col-xl-2 col-md-6">
                <label class="prime-field-label">Membros</label>
                <select name="members_count" class="prime-field">
            ...
            <div class="col-xl-2 col-md-6">
                ...
                <select class="prime-field" disabled title="CommunityGroup local não possui vínculo com plano">
                    <option>{{ $plans->count() ? 'Sem vínculo local' : 'Sem planos locais' }}</option>
                </select>
            </div>
            <div class="col-xl-2 col-md-6">
                <label class="prime-field-label">Sexo</label>
                <select class="prime-field" disabled title="CommunityGroup local não possui vínculo com sexo">
                    <option>Sem vínculo local</option>
                </select>
            </div>
            <div class="col-xl-2 col-md-6">
                <label class="prime-field-label">Sync</label>
                <select class="prime-field" disabled title="CommunityGroup local não possui flags de sincronização">
                    <option>Sem flags locais</option>
                </select>
            </div>
            <div class="col-xl-1 col-md-6 d-flex gap-2">
                <button type="submit" class="prime-btn-primary w-100" title="Filtrar">
                    <i class="ri-search-line"></i>
                </button>
                @if($filters['q'] !== '' || filled($filters['members_count']))
                    <a href="{{ url()->current() }}" class="prime-btn-ghost" title="Limpar filtros">
    ...
    <div class="prime-client-list">
        @forelse($groups as $group)
            <a href="{{ route('community.show', $group) }}" class="prime-client-card text-decoration-none">
                <div class="prime-client-card__main">
                    ...
                    <div class="prime-client-card__identity">
                        <div class="prime-client-card__name">{{ $group->name }}</div>
                        <div class="prime-client-card__meta">
                            <span>{{ $group->members_count ?? 0 }} membros</span>
                        </div>
                        @if($group->description)
                            <p class="prime-hub-excerpt">{{ Str::limit($group->description, 120) }}</p>
                        @else
                            <p class="prime-hub-excerpt">Grupo local sem descrição cadastrada.</p>
                        @endif
                    </div>
                </div>
        ...
        @empty
            <div class="prime-empty-state">
                <i class="ri-team-line"></i>
                <p>{{ ($filters['q'] !== '' || filled($filters['members_count'])) ? 'Nenhum grupo encontrado com os filtros atuais.' : 'Nenhum grupo cadastrado ainda. Crie o primeiro grupo para reunir seus alunos.' }}</p>
                <button type="button" class="prime-btn-primary" data-bs-toggle="modal" data-bs-target="#newGroupModal">
                    <i class="ri-add-line"></i> Novo Grupo
                </button>
            </div>
    ...
    @if($groups->hasPages())
        <div class="prime-pagination">{{ $groups->links() }}</div>
    @endif
</div>
        ...
        <form method="POST" action="{{ route('community.groups.store') }}" class="modal-content">
            ...
            <div class="modal-body">
                ...
                <div class="mb-3">
                    ...
                    <textarea name="description" class="prime-field" rows="3"></textarea>
                </div>
            </div>
