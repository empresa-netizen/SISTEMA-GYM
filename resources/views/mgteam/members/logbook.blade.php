@extends('layouts.master')

@section('title', 'Diário de registros')

@section('content')
@php
    $filtersOpen = request()->hasAny(['q', 'date', 'has_feedback']);
    $tabs = [
        'TRAINING' => ['label' => 'Registros de treino', 'icon' => 'ri-dumbbell-line'],
        'DIET' => ['label' => 'Registros de dieta', 'icon' => 'ri-restaurant-2-line'],
        'CARDIO' => ['label' => 'Registros de cardio', 'icon' => 'ri-heart-pulse-line'],
        'WEIGHT' => ['label' => 'Peso corporal', 'icon' => 'ri-scales-3-line'],
    ];
    $typeStyles = [
        'TRAINING' => ['label' => 'Treino', 'icon' => 'ri-dumbbell-line', 'chip' => 'mg-chip--info', 'gradient' => 'linear-gradient(135deg,#1d4ed8,#3b82f6)'],
        'DIET' => ['label' => 'Dieta', 'icon' => 'ri-restaurant-2-line', 'chip' => 'mg-chip--success', 'gradient' => 'linear-gradient(135deg,#15803d,#22c55e)'],
        'CARDIO' => ['label' => 'Cardio', 'icon' => 'ri-heart-pulse-line', 'chip' => 'mg-chip--danger', 'gradient' => 'linear-gradient(135deg,#be123c,#fb7185)'],
        'WEIGHT' => ['label' => 'Peso', 'icon' => 'ri-scales-3-line', 'chip' => 'mg-chip--warn', 'gradient' => 'linear-gradient(135deg,#7c3aed,#a855f7)'],
    ];
    $sourceLabels = [
        'student_weight_quick_log' => ['Peso no app', 'ri-smartphone-line'],
        'student_diet_free_meal' => ['Refeição livre', 'ri-restaurant-line'],
        'student_diet_meal_complete' => ['Refeição concluída', 'ri-checkbox-circle-line'],
        'student_workout_complete' => ['Treino concluído', 'ri-run-line'],
    ];
@endphp

<div class="mg-clients-page">
    <div class="mg-clients-toolbar">
        <div class="mg-clients-toolbar__left">
            <h1 class="mg-page-title mb-0">Diário de registros</h1>
            <div class="mg-clients-counters">
                <span class="mg-clients-counter">
                    <i class="ri-book-2-line"></i>
                    {{ array_sum($counts) }} registros
                </span>
            </div>
        </div>
        <div class="mg-clients-toolbar__right">
            <a href="{{ route('members.index') }}" class="mg-btn-ghost">
                <i class="ri-group-line"></i> Clientes ativos
            </a>
        </div>
    </div>

    @if(session('success'))
        <div class="alert alert-success alert-dismissible fade show" role="alert">
            {{ session('success') }}
            <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
        </div>
    @endif

    <p class="mg-page-sub mb-0">Acompanhe registros de treino e dieta enviados pelos clientes.</p>

    <div class="mg-segment-tabs" role="tablist">
        @foreach($tabs as $type => $tabConfig)
            <a href="{{ route('members.logbook', array_merge(request()->except(['page', 'type']), ['type' => $type])) }}"
               class="mg-segment-tab @if($activeType === $type) is-active @endif">
                <i class="{{ $tabConfig['icon'] }}"></i>
                {{ $tabConfig['label'] }}
                <span class="mg-segment-tab__count">{{ $counts[$type] ?? 0 }}</span>
            </a>
        @endforeach
    </div>

    <div class="mg-clients-filters">
        <button type="button" class="mg-btn-ghost mg-filters-toggle {{ $filtersOpen ? 'is-open' : '' }}" data-bs-toggle="collapse" data-bs-target="#mgLogbookFilters" aria-expanded="{{ $filtersOpen ? 'true' : 'false' }}">
            <i class="ri-filter-3-line"></i> Filtros
            <i class="ri-arrow-down-s-line mg-filters-chevron"></i>
        </button>
        <div class="collapse {{ $filtersOpen ? 'show' : '' }}" id="mgLogbookFilters">
            <form action="{{ route('members.logbook') }}" method="get" class="mg-clients-filters__form">
                <input type="hidden" name="type" value="{{ $activeType }}">
                <div class="mg-clients-filters__grid">
                    <div>
                        <label class="mg-field-label">Buscar</label>
                        <input type="search" name="q" value="{{ request('q') }}" class="mg-field" placeholder="Nome ou e-mail do cliente...">
                    </div>
                    <div>
                        <label class="mg-field-label">Data</label>
                        <input type="date" name="date" value="{{ request('date') }}" class="mg-field">
                    </div>
                    <div>
                        <label class="mg-field-label">Possui feedback</label>
                        <select name="has_feedback" class="mg-field">
                            <option value="">Todos</option>
                            <option value="yes" @selected(request('has_feedback') === 'yes')>Com feedback</option>
                            <option value="no" @selected(request('has_feedback') === 'no')>Sem feedback</option>
                        </select>
                    </div>
                    <div class="mg-clients-filters__actions">
                        <button type="submit" class="mg-btn-primary"><i class="ri-search-line"></i> Aplicar</button>
                        <a href="{{ route('members.logbook', ['type' => $activeType]) }}" class="mg-btn-ghost">Limpar</a>
                    </div>
                </div>
            </form>
        </div>
    </div>

    <div class="mg-client-list">
        @forelse($entries as $entry)
            @php
                $style = $typeStyles[$entry->type] ?? $typeStyles['TRAINING'];
                $name = $entry->member?->name ?? 'Cliente removido';
                $initials = collect(explode(' ', $name))->map(fn ($w) => mb_substr($w, 0, 1))->take(2)->implode('');
                $hasFeedback = $entry->member?->feedbacks?->isNotEmpty() ?? false;
                $metadata = is_array($entry->metadata) ? $entry->metadata : [];
                $source = $metadata['source'] ?? null;
                [$sourceLabel, $sourceIcon] = $sourceLabels[$source] ?? [null, null];
            @endphp
            <div class="mg-client-card">
                <div class="mg-client-card__main">
                    <div class="mg-client-card__avatar" style="background:{{ $style['gradient'] }}">
                        <i class="{{ $style['icon'] }}"></i>
                    </div>
                    <div class="mg-client-card__identity">
                        <div class="mg-client-card__name">{{ $entry->title ?: $style['label'] }}</div>
                        <div class="mg-client-card__meta">
                            <span>{{ $name }}</span>
                            @if($entry->member?->email)
                                <span class="mg-client-card__sep">|</span>
                                <span>{{ $entry->member->email }}</span>
                            @endif
                            <span class="mg-client-card__sep">|</span>
                            <span>{{ $entry->logged_at?->format('d/m/Y') ?? $entry->created_at?->format('d/m/Y') }}</span>
                        </div>
                        <div class="mg-client-chips">
                            <span class="mg-chip {{ $style['chip'] }}">{{ $style['label'] }}</span>
                            @if($entry->rating)
                                <span class="mg-chip mg-chip--warn">{{ $entry->rating }} ★</span>
                            @endif
                            @if($entry->numeric_value !== null)
                                <span class="mg-chip">
                                    {{ number_format((float) $entry->numeric_value, 2, ',', '.') }} {{ $entry->unit }}
                                </span>
                            @endif
                            @if($sourceLabel)
                                <span class="mg-chip mg-chip--info"><i class="{{ $sourceIcon }}"></i> {{ $sourceLabel }}</span>
                            @endif
                            <span class="mg-chip {{ $hasFeedback ? 'mg-chip--success' : '' }}">
                                {{ $hasFeedback ? 'Com feedback' : 'Sem feedback' }}
                            </span>
                        </div>
                        <p class="mg-hub-excerpt">{{ Str::limit($entry->comment ?: 'Sem observações no registro.', 150) }}</p>
                    </div>
                </div>
                <div class="mg-client-card__actions">
                    @if($entry->member)
                        <a href="{{ route('members.show', ['member' => $entry->member, 'tab' => 'logbook']) }}" class="mg-btn-ghost mg-btn-ghost--sm">
                            Visualizar
                        </a>
                    @endif
                    <form method="POST" action="{{ route('members.logbooks.destroy', $entry) }}" class="mg-inline-form" onsubmit="return confirm('Excluir este registro do diário?')">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="mg-btn-ghost mg-btn-ghost--sm text-danger">Excluir</button>
                    </form>
                </div>
            </div>
        @empty
            <div class="mg-empty-state">
                <i class="ri-book-2-line"></i>
                <p>Nenhum registro encontrado para esta aba.</p>
            </div>
        @endforelse
    </div>

    @if($entries->hasPages())
        <div class="mg-pagination">{{ $entries->links() }}</div>
    @endif
</div>
@endsection
