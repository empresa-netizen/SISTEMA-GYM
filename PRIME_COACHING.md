# Prime Coaching — Clone Local

Clone funcional do [Prime Coaching](https://app.primecoaching.com.br) baseado no GymFlow (Laravel 12).
...
**Login:** `coach@primecoaching.com.br` / `password`

## Plano da semana (D1–D7)

| Dia | Escopo | Status |
|-----|--------|--------|
| **D1** | Infra Docker, auth Prime, landing, dashboard, biblioteca Vimeo | ✅ |
...
```bash
docker compose exec -T app php artisan db:seed --class=PrimeCoachPaymentSeeder --force
docker compose exec -T app php artisan db:seed --class=PrimeCoachEventSeeder --force
docker compose exec -T app php artisan db:seed --class=PrimeCoachSupportSeeder --force
docker compose exec -T app php artisan db:seed --class=PrimeCoachCrmSeeder --force
```
...
Seeder CRM: `PrimeCoachCrmSeeder` (anamnese, diário, dieta demo, eventos com `member_id`).
...
| `.cursor/rules/prime-coaching-clone.mdc` | Regras Cursor do clone |

**Não portar código React/Expo** — portar contratos API, tokens MGTEAM (`--mgteam-*` em `prime-app.css`) e padrões de tela.
